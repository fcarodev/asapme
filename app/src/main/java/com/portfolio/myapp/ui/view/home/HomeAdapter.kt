...
class HomeAdapter(private val context:Context,var itemClickListener: ProjectClickListener): RecyclerView.Adapter<HomeViewHolder<*>>() {


    private var projectList = mutableListOf<ProjectModel>() //retorna un ArrayList

     fun setListProject(projectModels:MutableList<ProjectModel>) { projectList = projectModels }

    override fun getItemViewType(position: Int): Int {
        if (projectList[0].name == "EmptyDataProject") { return 0 }
        if (projectList[0].name == "PlaceholderDataProject") { return 1 }
        return 2
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HomeViewHolder<*> {
        return when (viewType) {
            0 -> { ProjectEmptyData(LayoutInflater.from(parent.context).inflate(R.layout.item_row_empty_data_home, parent, false)) }
            1 -> { ProjectPlaceHolderData(LayoutInflater.from(parent.context).inflate(R.layout.item_row_placeholder_home, parent, false)) }
            2 -> { ProjectViewHolderData(LayoutInflater.from(parent.context).inflate(R.layout.item_row_home, parent, false)) }
    ...
    override fun onBindViewHolder(holder: HomeViewHolder<*>, position: Int) {
        when (holder) {
            is ProjectEmptyData -> holder.bindView(ProjectModel("EmptyDataProject"), itemClickListener)
            is ProjectPlaceHolderData -> holder.bindView(ProjectModel("PlaceholderDataProject"), itemClickListener)
    ...
    override fun getItemCount(): Int {
        if (projectList[0].name.equals("EmptyDataProject")) { return 1 }
        if (projectList[0].name.equals("PlaceholderDataProject")) { return 10 }
